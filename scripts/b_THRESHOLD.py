...
def roc_points_from_scores(y_true: np.ndarray, scores: np.ndarray):
    """ROC curve by thresholding scores (>= t). Returns array: FPR, TPR, threshold."""
    y_true = np.asarray(y_true).astype(int)
    scores = np.asarray(scores).astype(float)
...
def pr_points_from_scores(y_true: np.ndarray, scores: np.ndarray):
    ...
    Returns arrays: recall, precision, threshold (sorted by recall ascending).
    """
    y_true = np.asarray(y_true).astype(int)
    scores = np.asarray(scores).astype(float)
    ...
    for t in ts:
        ...
        tp, tn, fp, fn = confusion_counts(y_true, y_pred)
        sens, spec, prec, f1, fpr, acc, youden = metrics_from_counts(tp, tn, fp, fn)
        rows.append((sens, prec, t))  # sens = recall
...
def average_precision(y_true: np.ndarray, scores: np.ndarray) -> float:
    """
    Average Precision (AP) like sklearn:
    sort by score desc, then AP = Σ (Δrecall * precision_at_step)
    """
    y_true = np.asarray(y_true).astype(int)
    scores = np.asarray(scores).astype(float)
...
def breadth_of_coverage(bam: Path, mapq: int, depth_min: int = 1, method: str = "coverage"):
    ...
    method:
    - "coverage": uses samtools coverage (fast)
...
def load_truth(truth_path: Path) -> pd.DataFrame:
    """Needs columns: sample, truth_cmv_present (or cmv_present)"""
...
def pick_score(df: pd.DataFrame, metric: str):
    ...
    if metric == "combo":
        return np.log1p(df["cmv_mapped_reads_mapq"].to_numpy(dtype=float)) + 10.0 * df["cmv_breadth_mapq"].to_numpy(
            dtype=float
        )
...
def threshold_scan_2d(y, mapped_hq, second, second_name: str):
    ...
    Returns dataframe + unique threshold grids.
    """
...
def save_2d_heatmap(
    ...
    plt.title(
        f"2D threshold scan heatmap: {value_col}\n"
...
def main():
    ...
    ap.add_argument(
        "--metric",
        choices=["mapped_hq", "rpm_hq", "breadth", "cov_bases", "combo"],
        default="mapped_hq",
        help="Which alignment metric to use as 1D ROC/PR score.",
    )
    ap.add_argument(
        "--rule",
        choices=["youden", "minscore_sens_fpr"],
        default="youden",
    ...
    ap.add_argument(
        "--two-dim",
        choices=["none", "mapped_breadth", "mapped_covbases"],
        default="none",
        help="Run 2D threshold search: mapped_hq + breadth OR mapped_hq + covered_bases.",
    )
    ap.add_argument(
        "--rule2d",
        choices=["youden", "max_f1", "minscore_sens_fpr"],
        default="youden",
        help="Selection rule for 2D thresholds.",
    )
    ap.add_argument("--top2d", type=int, default=50, help="Write top-N 2D threshold pairs by objective.")
    args = ap.parse_args()
    ...
    near_equal = (
        (align_df["total_reads_in_cmv_bam"] <= (align_df["cmv_mapped_reads_mapq"] + 2))
        & (align_df["cmv_mapped_reads_mapq"] > 0)
    ).mean()
    if near_equal > 0.30:
    ...
    if args.two_dim != "none":
        ...
        if args.rule2d == "youden":
            ...
            scan2d["objective"] = scan2d["F1"].fillna(-1e18)
        else:
            scan2d["objective"] = scan2d["youden_J"].fillna(-1e18)

        scan2d_sorted = scan2d.sort_values("objective", ascending=False).reset_index(drop=True)
        scan2d_sorted.head(int(args.top2d)).to_csv(
            outdir / f"threshold_scan_2d_top{args.top2d}_{args.two_dim}.tsv", sep="\t", index=False
        )
    ...
    with open(outdir / "report_alignment_thresholding.txt", "w") as f:
        ...
        if args.two_dim != "none":
            ...
            if best2d is None:
                ...
                f.write(
                    f"Sensitivity={best2d['sensitivity']:.4g} Specificity={best2d['specificity']:.4g} "
                    f"FPR={best2d['FPR']:.4g}\n"
                )
